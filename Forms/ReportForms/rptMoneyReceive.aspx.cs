...
{
    public partial class rptMoneyReceive : Page
    {
        SqlConnection con;
        SqlCommand Cmd;
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        ((TextObject)AppEnv.Current.p_rptSource.ReportDefinition.Sections["Section1"].ReportObjects["txtDateRange"]).Text = "Money Receive Info: Date From " + String.Format("{0:dd/MM/yyyy}", dpStartDate.SelectedDate) + " To " + String.Format("{0:dd/MM/yyyy}", dpEndDate.SelectedDate);
